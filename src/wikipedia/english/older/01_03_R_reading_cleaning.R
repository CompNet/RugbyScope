###########################################################################
## Project: RugbyScope
## Script purpose: load in the data
## Date: 26-11-2025
...
data = jsonlite::read_json("data/wikipedia/english/all_wikidata_players_profile.json")
class(data)
...
for(row_i in 1:length(data_df)){
  
  data_i <- data_df$data[row_i][[1]]
  data_df$data_v2[row_i] <- data_i
...
# Read JSON
data <- read_json("data/wikipedia/english/all_wikidata_players_profile.json")

data

# Convert to tibble with nested lists preserved
players_tbl <- map_dfr(data, function(x) {
  tibble(
    name = x$name %||% NA,
    successfully_scrape = x$successfully_scrape %||% NA,
    date_of_birth = list(x$date_of_birth),       # wrap in list
    date_of_death = list(x$date_of_death),       # wrap in list
    place_of_birth = x$place_of_birth %||% NA,
    place_of_death = x$place_of_death %||% NA,
    height = list(x$height),                     # wrap in list
    weight = list(x$weight),                     # wrap in list
    education = list(x$education),               # wrap in list
    position = list(x$position),                 # wrap in list
    career = list(x$career),                     # wrap in list
    player_html = x$player_html %||% NA,
    accessed_at = x$accessed_at %||% NA,
    req_alt_scrape = x$req_alt_scrape %||% NA,
    profile_url = x$profile_url %||% NA
  )
})
...
test |> print(n = Inf)
  ...
  rowwise() |> 
  mutate(a = stringr::str_split(position, "[/;]")) |> 
  print(n = Inf) 


s <- "wing/fullback;outside centre"
stringr::str_split(s, "[;/]")
# look at height and weight -----------------------------------------------


val <- 
  players_tbl |> unnest(height) |> unnest(height) |> 
  filter(height > 1.4, height < 2.5) |> pull(height)
val[val != 0]  |> hist()


players_tbl |> unnest(position) |> unnest(position) |> 
  count(position) |> arrange(desc(n)) |> print(n = Inf)


test <- players_tbl |> unnest(height) |> unnest(height) |> 
  filter(height > 2.5)

test$profile_url[6]
test$height[6]

players_tbl$position[1000]


# use vincent code as a base----------------------------------------------------

get_clean_positions()
test <- test |> 
  rename("positions" = "position")  
