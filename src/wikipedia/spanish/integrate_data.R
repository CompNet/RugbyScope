########################################################################
# Loads the clean Spanish Wikipedia tables and inserts them into our
# merged tables.
#
...
########################################################################
# paths
wp_folder <- file.path("data", "wikipedia", "spanish")
#
...
fus_teams <- read.csv(file.path(fusion_folder, "teams_05_itwp.csv"))
tlog(2, "Raw number of teams: ", nrow(fus_teams))

fus_players <- read.csv(file.path(fusion_folder, "players_04_itwp.csv"))
tlog(2, "Raw number of players: ", nrow(fus_players))

fus_stints <- read.csv(file.path(fusion_folder, "stints_03_itwp.csv"))
...
es_names <- wp_players[, "esName"]
es_full_names <- wp_players[, "esFullName"]
# loop over players to copy WP data
for (p in 1:length(idx)) {
  if (!is.na(es_names[p]) && es_names[p] != full_names[idx[p]]) {
    # possibly complement list of alt names
    if (all(is.na(alt_names[[idx[p]]])))
      a_names <- es_names[p]
    else
      a_names <- union(alt_names[[idx[p]]], es_names[p])
    if (!is.na(es_full_names[p]))
      a_names <- union(a_names, es_full_names[p])
    a_names <- setdiff(a_names, full_names[idx[p]])
    # update in table
    ...
    alt_names_new <- paste(sort(a_names), collapse = "; ")
    }
...
tab.file <- file.path(fusion_folder, "players_05_eswp.csv")
...
del_rows <- which(grepl("\\b7\\b", wp_teams[, "altNames"], fixed = FALSE) | grepl("[\\W_/,.]7[\\W_/,.]", wp_teams[, "teamWP"], fixed = FALSE))
...
tlog(2, "Use manually defined name2id map to match certain teams based on their name")
...
# add merged table name to ES entry, for visual verification
...
#  print(wp_teams[ii, ])
#}
#### there are many cases: it is expected, there are many slightly different spanish names
...
# complement alternative names in merged table, using spanish names
...
# record merged table as a new CSV file
tab.file <- file.path(fusion_folder, "teams_06_eswp.csv")
tlog(2, "Recording as a CSV file: \"", tab.file, "\"")
    ...
    if (length(idx2) > 0) {
      ...
      if (update) {
        ...
        if (!all(is.na(agreements)) && any(agreements, na.rm = TRUE))
          fus_stints[idx3, "dataSource"] <- paste0(fus_stints[idx3, "dataSource"], "; esWP")
...
tab.file <- file.path(fusion_folder, "stints_04_eswp.csv")
tlog(2, "Recording as a CSV file: \"", tab.file, "\"")
