########################################################################
# Loads the clean Italian Wikipedia tables and inserts them into our
# merged tables.
#
...
########################################################################
# paths
wp_folder <- file.path("data", "wikipedia", "italian")
#
...
fus_players <- read.csv(file.path(fusion_folder, "players_03_frwp.csv"))
tlog(2, "Raw number of players: ", nrow(fus_players))

fus_stints <- read.csv(file.path(fusion_folder, "stints_02_frwp.csv"))
...
tab.file <- file.path(fusion_folder, "players_04_itwp.csv")
...
# add merged table name to IT entry, for visual verification
...
#  print(wp_teams[ii, ])
#}
#### there are many cases: it is expected, there are many slightly different italian names
...
# complement alternative names in merged table, using italian names
...
# record merged table as a new CSV file
tab.file <- file.path(fusion_folder, "teams_05_itwp.csv")
tlog(2, "Recording as a CSV file: \"", tab.file, "\"")
...
tab.file <- file.path(fusion_folder, "stints_03_itwp.csv")
tlog(2, "Recording as a CSV file: \"", tab.file, "\"")
