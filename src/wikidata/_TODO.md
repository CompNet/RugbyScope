* manually check that all teams from main pro leagues are present
  ex. most of the English Championship teams seem missing (no league indicated on WD, so they get filtered out)

* network extraction
  * improve removal of amateur clubs
    currently removing clubs without an affiliation and a competition
    but that may be too strict (ex. it removes many English clubs)
